...
function isArticleCategory(v: unknown): v is ArticleCategory {
  return v === 'literature' || v === 'science' || v === 'history' || v === 'humanities'
}

// Migration 028'dan keyin articles jadvalida .category ustuni bor;
// admin form to'ldirilgan article'lar shu ustundan foydalanadi. Eski
// article'lar yoki hali kategoriya belgilanmagan qatorlar uchun
// deriveCategory fallback qiladi -- sahifada barcha article rangli
// ko'rinadi.
export function articleCategoryFor(article: { id: string; category?: string | null }): ArticleCategory {
  return isArticleCategory(article.category) ? article.category : deriveCategory(article.id)
}

// Xuddi shunday read_time bilan -- backend'dagi qiymat ustunlik
// qiladi; bo'lmasa deterministic derive.
export function articleReadMinutesFor(article: { id: string; read_time?: number | null }): number {
  if (typeof article.read_time === 'number' && article.read_time > 0) return article.read_time
  return deriveReadingMinutes(article.id)
}
