...
const CATEGORIES = ['all', 'Task 1', 'Task 2']
...
export default function WritingCollocationPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (levelTab !== 'all') list = list.filter(w => w.level.toLowerCase() === levelTab.toLowerCase())
    ...
    <div className="p-6 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-start gap-4 mb-5">
          ...
          <div>
            <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{t('vocabulary.writingCollocations')}</h1>
      ...
      {!isSavedTab && (
        <div className="flex flex-wrap gap-1.5 mb-6">
          {CATEGORIES.map(cat => {
            ...
            const n      = cat === 'all' ? words.length : (catCounts[cat] ?? 0)
            return (
              <button key={cat} onClick={() => setCatTab(cat)}
                ...
                {cat === 'all' ? t('common.all') : cat}{n > 0 ? ` (${n})` : ''}
              </button>
            )
          })}
        </div>
