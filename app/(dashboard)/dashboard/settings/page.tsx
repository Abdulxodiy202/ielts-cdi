...
export default function SettingsPage() {
  ...
  async function toggle() {
    ...
    try {
      ...
      if (next) {
        ...
        setToast(t('settingsPage.planReadyToast'))
  ...
  async function changeMode(next: Mode) {
    ...
    try {
      ...
      const { error: genErr } = await supabase.rpc('generate_plan_for_user', { p_user_id: user.id })
      if (genErr) console.error('[settings] generate_plan_for_user failed:', genErr.message)
      setToast(t('settingsPage.modeUpdatedToast'))
      setTimeout(() => setToast(null), 4000)
    } finally {
      setBusy(false)
    }
    ...
    <div className="p-6 md:p-8 max-w-2xl mx-auto">
      ...
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold flex items-center gap-2" style={{ color: 'var(--text-primary)' }}>
          <SettingsIcon size={26} style={{ color: 'var(--accent)' }} /> {t('settingsPage.title')}
        </h1>
      </div>
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="font-semibold text-lg mb-1" style={{ color: 'var(--text-primary)' }}>
              {t('settingsPage.dailyPlanTitle')}
            </h2>
            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
              {t('settingsPage.dailyPlanDesc')}
            </p>
          </div>
          <button
            type="button"
            ...
            aria-label={t('settingsPage.dailyPlanAria')}
          >
            <span
        ...
        {!isPremium && (
          ...
          >
            ...
            <span>{t('settingsPage.freeTierLock')}</span>
          </p>
        )}
      </div>
      ...
      {isPremium && (
        ...
        >
          <h2 className="font-semibold text-lg mb-1" style={{ color: 'var(--text-primary)' }}>
            {t('settingsPage.modeSectionTitle')}
          </h2>
          <p className="text-sm mb-4" style={{ color: 'var(--text-muted)' }}>
            {t('settingsPage.modeSectionDesc')}
          </p>
          <div className="grid grid-cols-2 gap-3">
            {([
              { key: 'weekly' as const, title: t('settingsPage.modeWeeklyTitle'), desc: t('settingsPage.modeWeeklyDesc') },
              { key: 'daily' as const,  title: t('settingsPage.modeAiDailyTitle'), desc: t('settingsPage.modeAiDailyDesc') },
            ]).map(o => {
