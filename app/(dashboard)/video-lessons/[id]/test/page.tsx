...
export default function VideoTestPage() {
  ...
  // Yulduz animatsiyasi: 0 → ... → finalStars, har 400ms. Tail
  // (score + best + tugmalar) yulduzlar tugagach fade-in bo'ladi.
  const [visibleStars, setVisibleStars] = useState(0)
  const [showResultTail, setShowResultTail] = useState(false)
  ...
  // Result animatsiya + 5⭐ confetti. Yulduzlar 500ms boshlang'ich
  // pauzadan keyin har 400ms'da bittasi yonadi. Tail (score/best/
  // tugmalar) yulduzlar tugagach chiqadi. 5⭐ bo'lsa oxirgi yulduzdan
  // ~300ms keyin confetti otiladi. firedRef ikki marta otishning
  // oldini oladi -- retake 5⭐ bo'lsa uni handleRetry qayta ochadi.
  useEffect(() => {
    if (phase !== 'result') return
    setVisibleStars(0)
    setShowResultTail(false)
    const timers: ReturnType<typeof setTimeout>[] = []
    timers.push(setTimeout(() => {
      for (let i = 1; i <= finalStars; i++) {
        timers.push(setTimeout(() => setVisibleStars(i), (i - 1) * 400))
      }
      const tailDelay = Math.max(finalStars, 1) * 400 + 200
      timers.push(setTimeout(() => setShowResultTail(true), tailDelay))
      if (finalStars === 5 && !firedRef.current) {
        firedRef.current = true
        timers.push(setTimeout(() => {
          requestAnimationFrame(() => fireCelebrationConfetti(resultRef.current))
        }, finalStars * 400 + 300))
      }
    }, 500))
  ...
  async function saveResult(score: number, stars: number) {
    ...
    try {
      ...
      if (!existing) {
        await supabase.from('video_test_results').insert({
          user_id: user.id,
          video_id: videoId,
          best_score: score,
          best_stars: stars,
        ...
        await supabase.from('video_test_results').update({
  ...
  function handleFinish() {
    ...
    // Optimistically reflect the new best on the result screen without
    // waiting for a re-fetch.
    setPreviousBest(prev => ({
      best_score: Math.max(prev?.best_score ?? 0, correct),
      best_stars: Math.max(prev?.best_stars ?? 0, stars),
    }))
    setPhase('result')
  }
    ...
    return (
      <div className="p-6 md:p-10 max-w-2xl mx-auto text-center">
        ...
        <h1 className="text-xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>
          {t('videoTest.unavailableTitle')}
        </h1>
        <p className="mb-6" style={{ color: 'var(--text-muted)' }}>
          {t('videoTest.unavailableDesc')}
        </p>
        <Link
          href="/video-lessons"
        ...
        >
          <ArrowLeft size={15} /> {t('videoTest.backToVideos')}
        </Link>
      </div>
    )
  }
  ...
  if (phase === 'quiz' && currentQuestion) {
    ...
    return (
      <div className="p-4 md:p-6 max-w-2xl mx-auto">
        ...
        >
          <ChevronLeft size={14} /> {t('videoTest.backToVideos')}
        </Link>

        <div className="mb-4 flex items-center justify-between text-sm" style={{ color: 'var(--text-muted)' }}>
          <span>{t('videoTest.questionCounter', { current: currentIdx + 1, total: TOTAL_QUESTIONS })}</span>
          <span>{t('videoTest.answered', { count: answeredCount })}</span>
        </div>
        ...
        <div className="flex gap-3">
          ...
          >
            <ChevronLeft size={16} /> {t('videoTest.back')}
          </button>
          {isLastQuestion ? (
            <button
            ...
            >
              {t('videoTest.finish')} <ClipboardCheck size={16} />
            </button>
          ) : (
            <button
              ...
              style={{ background: 'var(--accent)', color: 'white' }}
            >
              {t('videoTest.next')} <ChevronRight size={16} />
            </button>
          )}
        </div>
      </div>
  ...
  if (phase === 'result') {
    const messageKey =
      finalStars === 5 ? 'videoTest.msg5' :
      finalStars === 4 ? 'videoTest.msg4' :
      finalStars === 3 ? 'videoTest.msg3' :
      finalStars === 2 ? 'videoTest.msg2' :
      finalStars === 1 ? 'videoTest.msg1' :
      'videoTest.msg0'
      ...
      <div className="p-6 md:p-10 max-w-xl mx-auto">
        ...
        >
          <p className="text-xs font-bold uppercase tracking-wide mb-3" style={{ color: 'var(--text-muted)' }}>
            {t('videoTest.yourResult')}
          </p>

          <div className="flex items-center justify-center gap-1 mb-4">
            {[1, 2, 3, 4, 5].map(i => {
              const lit = i <= visibleStars
              const justLit = i === visibleStars
              return (
                <Star
                  key={i}
                  size={36}
                  strokeWidth={1.5}
                  fill={lit ? '#fbbf24' : 'transparent'}
                  color={lit ? '#fbbf24' : 'var(--text-muted)'}
                  style={{
                    transition: 'color 300ms ease-out, fill 300ms ease-out',
                    animation: justLit ? 'starPop 400ms ease-out' : undefined,
                    filter: lit ? 'drop-shadow(0 0 10px rgba(251,191,36,0.55))' : undefined,
                    opacity: lit ? 1 : 0.55,
          ...
          <div style={{
            opacity: showResultTail ? 1 : 0,
            animation: showResultTail ? 'tailIn 350ms ease-out' : undefined,
            pointerEvents: showResultTail ? 'auto' : 'none',
          }}>
            ...
            <div className="flex gap-3">
              <button
                ...
                style={{ background: 'var(--bg-secondary)', color: 'var(--text-primary)', border: '1px solid var(--border)' }}
              >
                <RotateCcw size={15} /> {t('videoTest.retry')}
              </button>
              <button
                onClick={() => router.push('/video-lessons')}
                className="flex-1 flex items-center justify-center gap-1.5 py-3 rounded-xl text-sm font-bold"
                style={{ background: 'var(--accent)', color: 'white' }}
              >
                <ArrowLeft size={15} /> {t('videoTest.backToVideos')}
