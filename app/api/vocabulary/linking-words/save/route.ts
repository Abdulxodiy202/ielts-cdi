...
export async function POST(req: Request) {
  ...
  if (selectErr) {
    console.error('[LW save] select error:', selectErr.message)
  ...
  if (existing) {
    const { error: delErr } = await admin
      ...
      .eq('id', existing.id)
    if (delErr) {
      console.error('[LW save] delete error:', delErr.message)
    ...
    if (insErr) {
      console.error('[LW save] insert error:', insErr.message)
