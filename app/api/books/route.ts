...
export async function POST(req: NextRequest) {
  ...
  const { title, author, heyzine_url, pdf_url, source_type, is_premium, category } = body
  if (!title?.trim()) return NextResponse.json({ error: 'Title required' }, { status: 400 })

  // Manba turini normalize -- default 'heyzine' backward compat uchun.
  const type: 'heyzine' | 'pdf' =
    source_type === 'pdf' ? 'pdf' : 'heyzine'
  if (type === 'heyzine' && !heyzine_url?.trim()) {
    return NextResponse.json({ error: 'Heyzine URL required' }, { status: 400 })
  }
  if (type === 'pdf' && !pdf_url?.trim()) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      // Faqat aktiv manba maydonini yozamiz, ikkinchisi null qoladi.
      heyzine_url: type === 'heyzine' ? heyzine_url.trim() : null,
      pdf_url:     type === 'pdf' ? pdf_url.trim() : null,
      source_type: type,
