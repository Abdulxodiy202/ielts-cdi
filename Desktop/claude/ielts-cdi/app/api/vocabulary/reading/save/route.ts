...
export async function POST(req: Request) {
  ...
  if (selectErr) {
    console.error('[reading save] select error:', selectErr.message)
  ...
  if (existing) {
    const { error: delErr } = await admin
      ...
      .eq('id', existing.id)
    if (delErr) {
      console.error('[reading save] delete error:', delErr.message)
    ...
    if (insErr) {
      console.error('[reading save] insert error:', insErr.message)
