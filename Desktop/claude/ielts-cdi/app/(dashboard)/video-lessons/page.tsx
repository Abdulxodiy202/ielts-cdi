...
function VideoThumbnail({ video, userPremium }: { video: VideoLesson; userPremium: boolean }) {
  ...
  const thumbSrc = video.thumbnail_url || (ytId ? `https://img.youtube.com/vi/${ytId}/mqdefault.jpg` : null)
  ...
  return (
    <div style={{ position: 'relative', width: '100%', paddingTop: '56.25%', borderRadius: 12, overflow: 'hidden', background: 'var(--bg-secondary)' }}>
      ...
      {video.duration_minutes && (
        <span style={{ position: 'absolute', bottom: 8, right: 8, background: 'rgba(0,0,0,0.75)', color: '#fff', fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 6 }}>
      ...
      {!locked && (
        <div className="play-overlay" style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0, transition: 'opacity .2s' }}>
...
export default function VideoLessonsPage() {
  ...
  const filtered = useMemo(() => catTab === 'Barchasi' ? videos : videos.filter(v => v.category === catTab), [videos, catTab])
  ...
  return (
    <>
      ...
      <div className="p-6 md:p-8 max-w-5xl mx-auto">
        ...
        {/* Header — Books uslubida, back button yo'q */}
        <div className="mb-8">
                ...
                <div key={v.id}
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-bold text-sm leading-snug line-clamp-2" style={{ color: 'var(--text-primary)' }}>{v.title}</h3>
                      {v.is_premium && (
                    ...
                    {v.description && (
                      <p className="text-xs leading-relaxed line-clamp-2 mb-3" style={{ color: 'var(--text-muted)' }}>{v.description}</p>
                    )}
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: cc.bg, color: cc.color }}>
                      {v.category}
                    </span>
                  </div>
                </div>
