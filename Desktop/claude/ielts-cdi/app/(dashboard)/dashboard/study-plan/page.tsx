...
export default function StudyPlanPage() {
  ...
  const load = useCallback(async () => {
    ...
    // Free-daily plan lives on its own row (period_start = today, mode
    // in daily_free/daily_free_locked) and is checked FIRST -- for a
    // free user with daily_plan_enabled this is what they see. Falls
    // through to the legacy weekly/daily plan UI otherwise (premium).
    const [p, dailyFree, settingsRes, streakRes, bonusesRes, profileRes] = await Promise.all([
      fetchActivePlan(supabase, user.id),
      fetchTodayDailyFreePlan(supabase, user.id),
      ...
      supabase.from('profiles').select('is_premium, premium_until, daily_plan_enabled').eq('id', user.id).single(),
    ])
    ...
    // If the daily plan is enabled but today's row hasn't been generated
    // yet (cron hasn't fired, or the toggle was just flipped on), ask
    // the backend router to make one now. Guard with autoGenerated so a
    // rate-limited/failing RPC doesn't spin forever.
    const enabled = Boolean((profileRes.data as { daily_plan_enabled?: boolean } | null)?.daily_plan_enabled)
    if (enabled && !dailyFree && !autoGenerated) {
  ...
  // ── Daily-free branches (free users) take priority over the legacy
  //    weekly/daily UI so a free user always lands here first.

  // Toggle off -> nudge to Settings.
  if (!dailyEnabled && !plan) {
    return (
      <div className="p-6 md:p-8 max-w-2xl mx-auto">
        ...
        <div className="py-12 md:py-16 px-6 text-center rounded-2xl" style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          <div className="text-4xl mb-3">📋</div>
          <h1 className="text-xl md:text-2xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>
            Kunlik plan yoqilmagan
          </h1>
          <p className="text-sm mb-6" style={{ color: 'var(--text-muted)' }}>
            Har kuni sizga moslashtirilgan reja tayyorlanadi. Sozlamalardan yoqib qo&apos;ying.
          </p>
          <Link
            href="/dashboard/settings"
            className="inline-flex items-center gap-1.5 px-6 py-3 rounded-xl text-sm font-bold text-white"
            style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}
          >
            <Settings size={15} /> Sozlamalarga o&apos;tish
          </Link>
        </div>
      </div>
    )
  ...
  // Week 2+ upsell wall for free users.
  ...
  // Active week-1 daily-free plan -> the bento task grid.
  ...
  // ── Legacy weekly / premium-daily UI below ──
