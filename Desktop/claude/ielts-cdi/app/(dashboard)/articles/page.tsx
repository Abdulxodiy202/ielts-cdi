        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
          {articles.map(article => {
            ...
            return (
              ...
              >
                {/* ── TOP: Title + Badge ── */}
                <div className="flex items-start justify-between gap-2 sm:gap-3 px-3 pt-3 pb-2 sm:px-4 sm:pt-4 sm:pb-3 lg:px-5 lg:pt-5 lg:pb-4">
                  <h3
                    className="font-bold leading-snug line-clamp-2 text-sm sm:text-base lg:text-lg xl:text-xl"
                ...
                {/* ── MIDDLE: Cover image ── */}
                <div className="relative mx-3 sm:mx-4 lg:mx-5 aspect-[16/9] w-full overflow-hidden rounded-xl" style={{ flexShrink: 0 }}>
                  {article.cover_image_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={article.cover_image_url} alt={article.title} className="w-full h-full object-cover object-center" />
                  ) : (
                ...
                <div className="px-3 pt-2 pb-3 sm:px-4 sm:pt-3 sm:pb-4 lg:px-5 lg:pt-4 lg:pb-5" style={{ marginTop: 'auto' }}>
                  {locked ? (
                    <button
                      onClick={e => { e.stopPropagation(); router.push(`/articles/${article.id}`) }}
                      className="w-full flex items-center justify-center gap-2 py-2 sm:py-2.5 rounded-xl text-sm sm:text-base font-semibold"
                      style={{ background: 'rgba(245,158,11,0.1)', color: '#f59e0b', border: '1px solid rgba(245,158,11,0.3)' }}
                    >
                      <Lock size={13} /> {t('articles.premium')}
                    </button>
                  ) : !article.file_url ? (
                    <div className="w-full py-2 sm:py-2.5 text-center text-sm sm:text-base rounded-xl"
                      style={{ background: 'var(--bg-secondary)', color: 'var(--text-muted)', border: '1px solid var(--border)' }}>
                      {t('articles.soon')}
                    </div>
                  ) : (
                    <button
                      className="w-full flex items-center justify-center gap-1.5 py-2 sm:py-2.5 rounded-xl text-sm sm:text-base font-semibold"
