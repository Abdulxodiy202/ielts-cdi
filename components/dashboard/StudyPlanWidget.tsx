  ...
  if (!plan) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-center py-6">
        ...
        <p className="font-bold mb-1" style={{ color: 'var(--text-primary)' }}>{t('studyPlanWidget.empty')}</p>
        <p className="text-xs mb-4" style={{ color: 'var(--text-muted)' }}>
          {t('studyPlanWidget.emptyDesc')}
        </p>
        <button
        ...
        >
          {generating ? t('studyPlanWidget.generating') : t('studyPlanWidget.createPlan')}
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h2 className="text-lg font-semibold" style={{ color: 'var(--text-primary)' }}>{t('studyPlanWidget.title')}</h2>
        {plan.is_completed && (
          <span className="ml-auto text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: 'rgba(16,185,129,0.15)', color: '#10b981' }}>
            {t('studyPlanWidget.completed')}
          </span>
        )}
      </div>
      ...
      <div className="mb-4">
        ...
        <p className="text-xs font-semibold" style={{ color: 'var(--text-muted)' }}>{t('studyPlanWidget.percentDone', { n: percent })}</p>
      </div>
          ...
          return (
            <div key={r.label} className="flex items-center gap-2 text-sm">
              ...
              >
                ...
                {r.retry && !complete && <span title={t('studyPlanWidget.retryTooltip')}>🔄</span>}
                {r.label}
              </span>
      ...
      <div className="mt-3 flex items-center justify-between">
        <span className="text-xs font-semibold" style={{ color: '#f59e0b' }}>
          {t('studyPlanWidget.streak', { n: streak })}
        </span>
        <Link
        ...
        >
          {t('studyPlanWidget.viewDetails')} <ChevronRight size={14} />
        </Link>
      </div>
    </div>
  )
