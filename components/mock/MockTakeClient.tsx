  ...
  if (!fileUrl) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3"
        ...
        <p className="text-sm">{t('mockFlow.sectionFileNotUploaded')}</p>
      </div>
    )
  }
      ...
      <div style={{ position: 'relative', width: '100%', height: 'calc(100vh - 180px)', minHeight: 400 }}>
        ...
        {showDone && onDone && (
          <button
            ...
            {t('mockFlow.nextSection')} <ArrowLeft size={15} style={{ transform: 'rotate(180deg)' }} />
          </button>
        )}
      </div>
    )
    ...
    return (
      <div className="flex flex-col items-center justify-center py-12 gap-6">
        ...
        <p className="text-sm font-medium" style={{ color: 'var(--text-secondary)' }}>
          {t('mockFlow.playListeningPrompt')}
        </p>
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4">
        ...
        <h3 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>
          {t('mockFlow.writingSubmittedTitle')}
        </h3>
        <p className="text-sm text-center max-w-xs" style={{ color: 'var(--text-muted)' }}>
          {t('mockFlow.writingSubmittedBody')}
        </p>
        <Link href="/mock-test" className="btn-primary mt-2">
          <ArrowLeft size={15} /> {t('mockFlow.backToMockPage')}
        </Link>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="card p-5 space-y-4">
        ...
        {!schedule.writing_task1_image_url && (
          <div className="flex items-center gap-2 p-3 rounded-xl text-sm"
            ...
            <ImageIcon size={14} /> {t('mockFlow.imageNotUploaded')}
          </div>
        )}
      ...
      <button
        ...
        {submitting
          ? <><Loader2 size={18} className="animate-spin" /> {t('mockFlow.submittingLabel')}</>
          : <><Send size={18} /> {t('mockFlow.submitLabel')}</>}
      </button>
      ...
      {lightboxUrl && (
        ...
        >
          ...
          <img
            src={lightboxUrl}
            alt={t('mockFlow.taskImageAltLarge')}
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg-primary)' }}>
      ...
      <div className="sticky top-0 z-30 flex items-center justify-between px-4 py-3"
        ...
        <Link href="/mock-test"
          ...
          style={{ color: 'var(--text-muted)' }}>
          <ArrowLeft size={15} /> {t('mockFlow.exitMock')}
        </Link>
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-5">
            ...
            <div className="text-center max-w-xs">
              <h3 className="font-bold text-lg mb-2" style={{ color: 'var(--text-primary)' }}>
                {t('mockFlow.writingSectionTitle')}
              </h3>
              <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                {t('mockFlow.writingSectionHint')}
              </p>
            </div>
            <button
              ...
              <Clock size={16} /> {t('mockFlow.startTimer60')}
            </button>
          </div>
        )}
      </div>
