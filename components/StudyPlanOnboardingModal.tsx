...
// 'weekly' silently. All copy pulled from messages/{en,uz}.json via
// LanguageContext so the modal follows the user's language pick.
    ...
    >
      ...
      >
        ...
        {step === 'intro' && (
          <>
            <h2 className="text-xl font-bold mb-3" style={{ color: 'var(--text-primary)' }}>
              {t('studyPlanOnboarding.introTitle')}
            </h2>
            <div className="text-sm leading-relaxed mb-6 space-y-2" style={{ color: 'var(--text-secondary)' }}>
              <p>{t('studyPlanOnboarding.introDesc')}</p>
              <ul className="space-y-1 pl-1">
                <li>• {t('studyPlanOnboarding.introItem1')}</li>
                <li>• {t('studyPlanOnboarding.introItem2')}</li>
                <li>• {t('studyPlanOnboarding.introItem3')}</li>
                <li>• {t('studyPlanOnboarding.introItem4')}</li>
              </ul>
              <p>{t('studyPlanOnboarding.introInvite')}</p>
            </div>
            <button
              onClick={goNext}
            ...
            >
              {t('studyPlanOnboarding.startBtn')} <ChevronRight size={16} />
            </button>
          </>
        )}
        ...
        {step === 'typing' && (
          <>
            <h2 className="text-xl font-bold mb-2" style={{ color: 'var(--text-primary)' }}>
              {t('studyPlanOnboarding.typingTitle')}
            </h2>
            <p className="text-sm leading-relaxed mb-5" style={{ color: 'var(--text-muted)' }}>
              {t('studyPlanOnboarding.typingDesc')}
            </p>
            <div className="grid grid-cols-2 gap-3 mb-6">
              ...
              >
                ...
                <div className="font-bold text-sm" style={{ color: 'var(--text-primary)' }}>{t('studyPlanOnboarding.yesAdd')}</div>
                <div className="text-xs mt-0.5" style={{ color: 'var(--text-muted)' }}>{t('studyPlanOnboarding.recommendedNote')}</div>
              </button>
              <button
              ...
              >
                ...
                <div className="font-bold text-sm" style={{ color: 'var(--text-primary)' }}>{t('studyPlanOnboarding.noOption')}</div>
              </button>
            </div>
            <div className="flex gap-3">
              <button onClick={goBack} className="flex-1 py-3 rounded-xl text-sm font-semibold inline-flex items-center justify-center gap-1"
                ...
                <ChevronLeft size={15} /> {t('studyPlanOnboarding.backBtn')}
              </button>
              <button onClick={goNext} className="flex-1 py-3 rounded-xl text-sm font-bold text-white inline-flex items-center justify-center gap-1.5"
                style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
                {t('studyPlanOnboarding.nextBtn')} <ChevronRight size={15} />
              </button>
            </div>
          </>
        )}
        ...
        {step === 'confirm' && (
          <>
            <h2 className="text-xl font-bold mb-3" style={{ color: 'var(--text-primary)' }}>
              {t('studyPlanOnboarding.confirmTitle')}
            </h2>
            <div
              className="text-sm rounded-xl p-4 mb-5 space-y-1"
              style={{ background: 'var(--bg-secondary)', border: '1px solid var(--border)', color: 'var(--text-secondary)' }}
            >
              <p className="font-semibold mb-2" style={{ color: 'var(--text-primary)' }}>{t('studyPlanOnboarding.yourChoices')}</p>
              <p>• {t('studyPlanOnboarding.typingLabel')} <strong>{includeTyping ? t('studyPlanOnboarding.yesLabel') : t('studyPlanOnboarding.noLabel')}</strong></p>
              {isPremium && <p>• {t('studyPlanOnboarding.modeLabel')} <strong>{mode === 'weekly' ? t('studyPlanOnboarding.modeWeeklyLabel') : t('studyPlanOnboarding.modeDailyLabel')}</strong></p>}
              <p className="pt-2" style={{ color: 'var(--text-muted)' }}>{t('studyPlanOnboarding.firstPlanNote')}</p>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={finish} disabled={saving}
                ...
                style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)' }}>
                {saving ? t('studyPlanOnboarding.creating') : t('studyPlanOnboarding.createPlan')} {!saving && <ChevronRight size={15} />}
              </button>
            </div>
          </>
        )}
        ...
        {step === 'done' && (
          <div className="text-center py-8">
            ...
            <p className="text-lg font-bold" style={{ color: 'var(--text-primary)' }}>{t('studyPlanOnboarding.planCreated')}</p>
          </div>
        )}
      </div>
    </div>
